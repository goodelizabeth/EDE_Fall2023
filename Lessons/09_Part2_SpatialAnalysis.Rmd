...
huc8_UpperNeuse_sf<- st_read(here('Data/Spatial/NCHUC8.shp')) %>%
...
ggplot(huc8_sf) +
  geom_sf(data = huc8_sf, 
          fill = "orange") +
  geom_sf(data = huc8_UpperNeuse_sf, 
          fill = "purple")
  
mapView(huc8_sf, 
        col.region = "orange", 
        layer.name = "HUC 8") +
  mapView(huc8_UpperNeuse_sf, 
          col.region = "purple", 
          layer.name = "Upper Neuse")
...
When the data is served in GeoJSON format, we can ingest it directly in to R. Follow these steps:
  ...
  - Select the first return and open its [link](https://duke.is/8/jpzm).
  - Click the "I want to use this" button at the bottom left of the page.
  - Expand the "View API Resources" to view the link to the resource in GeoJSON format.
  - Copy the link to the GeoJSON option.

If you have difficulty, the link you want is: - <https://services1.arcgis.com/Hp6G80Pky0om7QvQ/arcgis/rest/services/Plants_gdb/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson>
This a link to a spatial dataset in GeoJSON format. Now let's read this dataset in and explore it:

```{r load.geojson.data}
powerplants_sf <- st_read('https://services1.arcgis.com/Hp6G80Pky0om7QvQ/arcgis/rest/services/Plants_gdb/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson')
...
svi2018_nc_raw <- read.csv(
  'https://svi.cdc.gov/Documents/Data/2018/csv/states_counties/NorthCarolina_county.csv',
