...
> Answer: 
It's really inconvienent to find out which row numbers correspond to your data of interest.
...
ex3 <- NTL.phys.data %>%
  filter(lakename =="Tuesday Lake") %>%
  filter(year4 %in% 1990:1999) %>%
  mutate(month = lubridate::month(sampledate)) %>%
  filter(month == 7)
...
ex4 <- ex3 %>%
  ...
  summarize(mean = mean(temperature_C))
...
NTL.phys.data.gathered <- 
  pivot_longer(NTL.phys.data,
               irradianceWater:irradianceDeck, names_to = "nutrient", values_to = "concentration")
