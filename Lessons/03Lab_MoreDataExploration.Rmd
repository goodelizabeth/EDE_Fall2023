...
vector1[3] # assess the third element of vector1 [1] 5.3

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # default of by column
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # make it populate by row 
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "quatro", "cinco"), 
                                  c("un", "deux", "trois", "quatre"))) # row titles and then column titles 
matrix3

matrix1[4, ] # get the 4th row 
matrix1[ , 3] # get the third column 
matrix1[c(12, 14)] # start counting by row until it gets to element 12 and then to element 14
matrix1[c(12:14)] # counts by row until it gets to elements 12 through 14
matrix1[2:4, 1:3] # rows 2 through 4 and columns 1 through 3 
...
high_temp_F <- c(81,82,84,84,77,72,79,77,78,78)
high_temp_F

low_temp_F <- c(53,55,58,59,61,54,62,63,62,62)
low_temp_F
```
...
high_temp_C <- F_to_C(high_temp_F)
high_temp_C
low_temp_C <- F_to_C(low_temp_F)
low_temp_C
```
...
```{r DataExercise-1c}
temperatures_df <- data.frame(high_temp_F, high_temp_C, low_temp_F, low_temp_C)
temperatures_df
```
