...
# for assignment - create sequence of dates from first to last observation in data set - left join seq of dates with data set - all rows in seq should be represented in data frame - then run summary and check for NAs
...
rainfall_ts <- ts(BR_complete$Rainfall, 
                  start = c(1901,1), 
                  frequency = 12)

temperature_ts <- ts(BR_complete$Temperature_C, 
                     start = c(1901,1), 
                     frequency = 12)
...
rainfall_decomposed <- stl(rainfall_ts, 
                           s.window = "periodic")

temperature_decomposed <- stl(temperature_ts, 
                           s.window = "periodic")
...
temperature_components <- as.data.frame(temperature_decomposed$time.series[,1:3])

rainfall_components <- 
  mutate(rainfall_components,
         Observed = BR_complete$Rainfall,     
         Date = BR_complete$Date)

temperature_components <- 
  mutate(temperature_components,
