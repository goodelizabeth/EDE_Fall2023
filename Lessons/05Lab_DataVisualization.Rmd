...
ggplot(PeterPaul.chem.nutrients,
       aes(x=(factor(PeterPaul.chem.nutrients$month,
                     levels = 1:12,
                     labels = month.abb)))) +
  geom_bar()
...
#Tidy up the code (make sure to return after commas, etc.)
...
# 1. 
# Plot surface temperatures (y) by day of  year (x). 
# Color your points by year, and facet by lake in two rows.
# Change the ylab name
surface_temp_plot <- PeterPaul.chem.nutrients %>% 
  filter(depth == 0) %>% 
  ggplot(
    mapping = aes(
      x=daynum,
  ...
  scale_y_continuous(name = "Surface Temperature (deg C)") +
...
temp_date_plot <- PeterPaul.chem.nutrients %>% 
  ggplot(
    mapping = aes(
      x=sampledate,
      y=temperature_C,
...
ozone_pm25_plot <- EPAair %>% 
  ...
  scale_y_continuous(name = "PM2.5") +
  scale_x_continuous(name = "Ozone") +
  labs(color='Latitude') +
  geom_point(alpha = 0.5) +
  geom_smooth(method = lm)

print(ozone_pm25_plot)
